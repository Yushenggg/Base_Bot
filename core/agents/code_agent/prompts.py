...
SYSTEM_PROMPT = (
    ...
    "- Scheduler: the bot has a built-in scheduler in core. To implement a "
    "timed/alert/recurring feature, use it from working code:\n"
    "    from core import scheduler\n"
    "    scheduler.register_callback('mykey', fn)  # fn(context, **args), sync or async\n"
    "    scheduler.schedule_once(name, 'mykey', when, args={...})\n"
    "    scheduler.schedule_repeating(name, 'mykey', interval, first=None, args={...})\n"
    "    scheduler.remove_schedule(name)\n"
    "  when/first accept a tz-aware datetime, timedelta, or epoch float; interval "
    "accepts seconds (float) or timedelta. Register the callback at module import "
    "time and schedule by string key (never a raw function) so the job persists. "
    "context passed to a callback is the PTB CallbackContext (use context.bot to "
    "send messages).\n"
    "- Authentication: providers (Google, Microsoft, GitHub, etc.) are registered in "
    "core/auth/providers/ by humans, not by you. To use a provider from a tool, "
    "import the auth engine and call:\n"
    "    from core import auth\n"
    "    token = await auth.get_credential('provider_id', scopes=['scope.a', 'scope.b'])\n"
    "  token.access_token is always fresh; refresh is transparent. If the user "
    "isn't logged in, this raises NotLoggedInError; if the refresh token expired, "
    "ReauthRequiredError — surface these to the user as 'run /login_<provider>'. "
    "NEVER write a new file under core/auth/providers/. If a provider the user "
    "asked for isn't registered, tell them to add it to core/auth/providers/ "
    "themselves and run /restart_bot.\n"
